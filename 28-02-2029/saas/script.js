const SaaSModule = {
    ...
    renderPlanButtons(product) {
        ...
        if (freeTier) {
            buttons += `<button class="btn btn-outline-light w-100 rounded-pill fw-bold mb-2" 
                onclick="window.open('${freeTier.action_url}', '_blank')">
                Start Free Trial
            </button>`;
        }

        buttons += `<button class="btn btn-warning w-100 rounded-pill fw-bold" 
