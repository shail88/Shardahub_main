            ...
            <div class="card h-100 glass-card course-card interactive-card">
                <a href="details.html?id=${course.id}" class="text-decoration-none">
                    <div class="course-img-wrapper">
                        ...
                        <img src="${sanitizeImg(course.thumbnail_url)}" class="card-img-top" alt="${course.title}" onerror="this.onerror=null; this.src=window.SHARDA_PLACEHOLDER;">
                    </div>
                </a>
...
async function loadCourseDetailsDB() {
    ...
    detailThumb.src = sanitizeImg(course.thumbnail_url);
    detailThumb.onerror = () => { detailThumb.onerror = null; detailThumb.src = window.SHARDA_PLACEHOLDER; };
    ...
    // Demo Button Logic
    const demoBtns = [demoBtn, document.getElementById('heroWatchDemoBtn')].filter(Boolean);
    if (course.demo_url && demoBtns.length > 0) {
        demoBtns.forEach(btn => {
            btn.style.display = 'inline-block';
            btn.onclick = () => {
                let finalUrl = course.demo_url;
                const ytRegex = /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=|shorts\/)([^#\&\?]*).*/;
                const ytMatch = finalUrl.match(ytRegex);
                const ytId = (ytMatch && ytMatch[2].length === 11) ? ytMatch[2] : null;

                if (ytId) finalUrl = `https://www.youtube.com/embed/${ytId}?autoplay=1&rel=0&origin=${window.location.origin}`;

                const demoWindow = window.open("", "_blank", "width=800,height=500");
                demoWindow.document.write(`
                    <head><title>Watch Demo: ${course.title}</title></head>
                    <body style="background:black;margin:0;display:flex;justify-content:center;align-items:center;height:100vh;">
                        <iframe width="100%" height="100%" src="${finalUrl}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>
                        <div style="position:absolute; bottom:10px; right:10px;"><a href="${course.demo_url}" target="_blank" style="color:white; font-family:sans-serif; text-decoration:none; font-size:12px; background:rgba(0,0,0,0.5); padding:5px 10px; border-radius:5px;">Watch on YouTube</a></div>
                    </body>
                `);
            };
        });
    ...
    if (typeof courses !== 'undefined') {
        ...
        container.innerHTML = similar.map(c => `
            <a href="details.html?id=${c.id}" class="glass p-3 rounded-4 d-flex align-items-center gap-3 text-white text-decoration-none similar-product-card transition-all mb-3">
                <img src="${sanitizeImg(c.thumbnail)}" width="60" height="40" class="rounded object-fit-cover" onerror="this.onerror=null; this.src=window.SHARDA_PLACEHOLDER;">
                <div>
        ...
        grid.innerHTML = displayCourses.map(course => `
            <div class="col-md-6 col-lg-4">
                <div class="card glass-card h-100 course-card">
                    <div class="course-img-wrapper">
                        <img src="${sanitizeImg(course.image_url || course.thumbnail_url)}" class="card-img-top" alt="${course.title}" onerror="this.onerror=null; this.src=window.SHARDA_PLACEHOLDER;">
...
async function toggleLesson(checkbox, lessonId) {
    ...
    const success = await ShardaDB.updateLessonProgress(user.id, courseId, lessonId, checkbox.checked);
    if (!success) {
...
function playLesson(lesson) {
    ...
    if (videoContainer) {
        if (lesson.content_url) {
            // Apply embed conversion
            const ytRegex = /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=|shorts\/)([^#\&\?]*).*/;
            const ytMatch = lesson.content_url.match(ytRegex);
            const ytId = (ytMatch && ytMatch[2].length === 11) ? ytMatch[2] : null;

            let finalUrl = lesson.content_url;
            if (ytId) {
                finalUrl = `https://www.youtube.com/embed/${ytId}?rel=0&origin=${window.location.origin}`;
            }

            videoContainer.innerHTML = `<iframe width="100%" height="100%" src="${finalUrl}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>`;
        } else {
...
async function startPayment() {
    ...
    if (typeof ShardaPayments !== 'undefined') {
        ...
        try {
            await ShardaPayments.initiateCheckout({
                amount: total,
                description: 'ShardaHub Course Order',
                section: 'course'
            });
            ShardaPayments.showToast("Payment Success!", "success");
            setTimeout(() => {
        ...
        alert("Payment system not found. Check if assets/js/payments.js is loaded.");
    }
}
