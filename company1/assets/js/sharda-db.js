...
const ShardaDB = {
    ...
    async getCourseById(id) {
        ...
        return {
            ...
            demo_url: data.products?.video_url,
    ...
    async updateLessonProgress(userId, courseId, lessonId, isCompleted, position = 0) {
    ...
    async getEnrollments(userId) {
        ...
        return (data || []).map(e => ({
            ...e,
            courses: e.courses ? { ...e.courses, ...e.courses.products } : null,
            products: e.courses?.products || null // Provide direct access for dashboard.html compatibility
        }));
    },
    ...
    async logAIGeneration(userId, toolId, prompt, creditsSpent = 1, status = 'success') {
        const { data, error } = await supabase.from('ai_generations').insert([{
            user_id: userId,
            tool_id: toolId,
            prompt_input: prompt?.substring(0, 500),
    ...
    async createSubscription(userId, productId, tierId, durationDays, razorpaySubId = null) {
        ...
        const { data, error } = await supabase.from('subscriptions').upsert([{
            ...
            tier_id: tierId,
            plan_name: 'Pro', // Default plan name if tier name not provided
            status: 'active',
    ...
    async getActiveSubscription(userId, productId) {
        const { data } = await supabase.from('subscriptions').select('*, product_tiers(name,features,daily_limit,monthly_limit)').eq('user_id', userId).eq('product_id', productId).eq('status', 'active').order('created_at', { ascending: false }).limit(1);
        return data?.[0] || null;
    },

    async cancelSubscription(userId, productId) {
        const { error } = await supabase.from('subscriptions').update({ status: 'cancelled' }).eq('user_id', userId).eq('product_id', productId).eq('status', 'active');
        return !error;
    },

    async getUserSubscriptions(userId) {
        const { data } = await supabase.from('subscriptions').select('*, products(id,title,image_url,product_type), product_tiers(name)').eq('user_id', userId).order('created_at', { ascending: false });
        return data || [];
    },

    // ═══════════════════════════════════════════════
    // GAME LICENSES (Uses Match History for basic tracking in this schema)
    // ═══════════════════════════════════════════════
    async grantGameLicense(userId, productId, tierId, maxDownloads = 3, gameVersion = '1.0') {
        // Falling back to match history or a simple record since licenses table isn't in current schema
        const { data, error } = await supabase.from('game_match_history').insert([{
            user_id: userId,
            game_id: productId,
            result: 'License Granted',
    ...
    async getGameLicenses(userId) {
        const { data } = await supabase.from('game_match_history').select('*, products(id,title,image_url)').eq('user_id', userId).order('played_at', { ascending: false });
        return data || [];
    },
    ...
    async getProfile(userId) {
        ...
        if (data && data.user_profiles) {
            Object.assign(data, Array.isArray(data.user_profiles) ? data.user_profiles[0] : data.user_profiles);
    ...
    async updateProfile(userId, updates) {
        ...
        for (const [k, v] of Object.entries(updates)) {
            // Include roles, avatars, verification in users table
            if (['full_name', 'avatar_url', 'role', 'is_verified'].includes(k)) {
                userUpdates[k] = v;
            } else {
                profileUpdates[k] = v;
            }
        ...
        if (Object.keys(profileUpdates).length > 0) {
            ...
            const { error } = await supabase.from('user_profiles').upsert([profileUpdates]);
            if (!uError && error) uError = error;
        }

        return { error: uError, data: updates };
    },

    async getDashboardStats(userId) {
        const [enrollRes, orderRes, subRes, matchRes] = await Promise.all([
            supabase.from('enrollments').select('course_id, progress_percent', { count: 'exact' }).eq('user_id', userId),
            supabase.from('orders').select('total_amount', { count: 'exact' }).eq('user_id', userId).eq('order_status', 'completed'),
            supabase.from('subscriptions').select('id', { count: 'exact' }).eq('user_id', userId).eq('status', 'active'),
            supabase.from('game_match_history').select('id', { count: 'exact' }).eq('user_id', userId),
        ]);
    ...
    async clearAllContacts() {
        // Technically this deletes everything. Safe only if used by admin.
        const { error } = await supabase
            .from('contacts')
            .delete()
    ...
    // ═══════════════════════════════════════════════
    // CURRICULUM MANAGEMENT (for Admin)
    // ═══════════════════════════════════════════════
