...
const GameModule = {
    ...
    async openGame(productId, tierId, actionUrl, isFree) {
        ...
        // Check subscription for paid tiers
        const user = window.SubpassAuth?.user;
        if (!user) {
            alert("Please login to access premium games.");
            window.location.href = '../login.html';
            return;
        }

        // Verify access via ShardaDB
        const hasAccess = await ShardaDB.checkEnrollment(user.id, productId); // simplified check, might need tier check
        if (hasAccess) {
            if (actionUrl) {
                window.open(actionUrl, '_blank');
            } else {
                alert("Access Granted! Game loading...");
            }
        } else {
