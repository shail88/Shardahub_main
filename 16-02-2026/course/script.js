...
async function loadCoursesFromDB() {
    ...
    try {
        ...
        console.error('ShardaDB failed, using dummy data');
        if (typeof courses !== 'undefined') coursesList = courses.map(c => ({
            id: c.id,
            title: c.title,
            category: c.category,
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <div class="d-flex justify-content-between align-items-center mt-3 pt-3 border-top border-secondary">
                        ...
                        <div class="d-flex gap-2">
                             ...
                             <button class="btn btn-sm btn-info rounded-pill px-3" onclick="event.preventDefault(); addToCart({id: '${course.id}', title: '${course.title.replace(/'/g, "\\'")}', price: ${course.price}, icon: 'bi-mortarboard'})">
...
async function handleFreeEnrollment(courseId) {
    ...
    const enrollments = ShardaDB.getTable('sharda_enrollments');
    const isEnrolled = enrollments.some(e => e.user_id === SubpassAuth.user.id && e.course_id === courseId);
    ...
    // Enroll
    await ShardaDB.enroll(SubpassAuth.user.id, courseId);
