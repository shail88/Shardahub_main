// Global Asset Helpers
const SHARDA_SVG_PLACEHOLDER = "data:image/svg+xml;base64,";
function sanitizeImg(url) {
    if (!url || url.includes('via.placeholder.com')) return SHARDA_SVG_PLACEHOLDER;
    return url;
}
            ...
            <div class="col-12 mb-3">
                <div class="glass p-3 rounded-4 d-flex align-items-center gap-4 interactive-card shadow-lg" style="border-left: 4px solid var(--ai-accent) !important;">
                    <div class="position-relative overflow-hidden rounded-3 shadow-sm" style="width: 120px; height: 80px;">
                        <img src="${sanitizeImg(p.image_url)}" class="w-100 h-100 object-fit-cover transition-base hover-bright" onerror="this.onerror=null; this.src=SHARDA_SVG_PLACEHOLDER;">
                    </div>
...
window.toggleCategoryFields = function () {
    ...
    const limitContainer = document.getElementById('tierLimitContainer');
    ...
    if (limitContainer) {
        // Show daily limit ONLY for AI or SaaS. Courses and Others don't need it.
        limitContainer.style.display = (slug === 'ai' || slug === 'saas') ? 'block' : 'none';
    }
}
...
window.editProduct = async function (id) {
    ...
    if (p) {
        ...
        document.getElementById('thumb_preview').src = sanitizeImg(thumbUrl);
        ...
        // Always start on Basic Info tab when opening edit
        const bTab = document.getElementById('basic-tab');
        if (bTab) bootstrap.Tab.getOrCreateInstance(bTab).show();
...
window.resetForm = function () {
    ...
    document.getElementById('thumb_preview').src = SHARDA_SVG_PLACEHOLDER;
}
        ...
        list.innerHTML = tiers.map(t => `
            <div class="d-flex align-items-center justify-content-between p-2 mb-2 bg-black bg-opacity-25 rounded border border-secondary">
                <div class="d-flex align-items-center flex-grow-1 gap-2">
                    <span class="badge bg-info text-dark fw-bold" style="min-width: 60px;">${t.name}</span>
                    <span class="text-white fw-medium">₹${t.price_inr}</span>
                    <span class="badge bg-secondary-subtle text-secondary small">Limit: ${t.daily_limit === -1 ? '∞' : t.daily_limit}</span>
                    <small class="text-secondary text-truncate d-none d-md-inline" style="max-width: 150px;">${t.action_url || ''}</small>
                </div>
                <button type="button" class="btn btn-sm btn-outline-danger py-0 border-0" onclick="removeTier('${t.id}', '${productId}')"><i class="bi bi-trash"></i></button>
            </div>
        `).join('');
    } else {
...
window.addTier = async function () {
    ...
    const catSlug = document.getElementById('category_slug').value; // Get the type
    ...
    const limitRaw = document.getElementById('tierLimit').value;
    ...
    const tierData = {
        ...
        name: name,
        price_inr: parseFloat(priceRaw) || 0,
        action_url: url || null,
        features: []
    };

    // ONLY add daily_limit if it's an AI or SaaS product
    if (catSlug === 'ai' || catSlug === 'saas') {
        tierData.daily_limit = parseInt(limitRaw) || 5;
    }
    ...
    if (contacts && contacts.length > 0) {
        container.innerHTML = contacts.map(c => `
            <div class="glass p-4 rounded-4 mb-4">
                ...
                <div class="p-3 bg-dark rounded-3 small position-relative">
                    <strong class="text-white d-block mb-1">Subject: ${c.subject || 'Inquiry'}</strong>
                    ${c.message}
                    <button class="btn btn-sm btn-outline-danger position-absolute top-0 end-0 m-2 border-0" onclick="deleteContact('${c.id}')"><i class="bi bi-trash"></i></button>
                </div>
            </div>
        `).join('');
    } else {
...
window.deleteContact = async function (id) {
    if (confirm("Delete this inquiry?")) {
        const ok = await ShardaDB.deleteContact(id);
        if (ok) loadContacts();
    }
}

window.clearAllInquiries = async function () {
    if (confirm("WARNING: Deep cleaning! This will permanently DELETE all contact inquiries. Continue?")) {
        const ok = await ShardaDB.clearAllContacts();
        if (ok) loadContacts();
    }
}
...
window.editModule = async function (id, currentTitle) {
    const { data: module } = await supabase.from('course_modules').select('*').eq('id', id).single();
    if (!module) return;

    const newTitle = prompt("Edit Section Title:", module.title);
...
window.editLesson = async function (id) {
    ...
    const { data: lesson } = await supabase.from('course_lessons').select('*').eq('id', id).single();
    if (!lesson) return;

    const title = prompt("Edit Title:", lesson.title);
    const url = prompt("Edit URL:", lesson.content_url);

    if (title === null) return; // User cancelled

    const { error } = await ShardaDB.upsertLesson({
        ...lesson, // Spread existing data to keep module_id, order_index, etc.
        title: title,
        content_url: url
    });
