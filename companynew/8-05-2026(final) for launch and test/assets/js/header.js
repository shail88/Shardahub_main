 ...
 */
var ShardaHeader = window.ShardaHeader || {
    init() {
        ...
        // Check for existing auth state in case we loaded after the event
        if (window.SubpassAuth && window.SubpassAuth.user) {
            this.updateUI({ user: window.SubpassAuth.user, metadata: window.SubpassAuth.metadata });
        }
    ...
    updateUI(data) {
        ...
        // Proactively calculate baseUrl if not provided by sidebar.js yet
        ...
        const safeBaseUrl = window.ShardaBaseUrl || detectedBaseUrl;

        if (data.user) {
            const userAvatar = data.user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(data.user.full_name || 'User')}&background=0ea5e9&color=fff`;

            container.innerHTML = `
                <div class="dropdown">
                    <img src="${userAvatar}" class="profile-avatar dropdown-toggle" id="profileDropdown" data-bs-toggle="dropdown">
                    <ul class="dropdown-menu dropdown-menu-end dropdown-menu-dark shadow border-secondary">
                        ...
                        ${data.user.role?.toLowerCase() === 'admin' ? `<li><a class="dropdown-item py-2 text-info fw-bold" href="${safeBaseUrl}admin/dashboard.html"><i class="bi bi-speedometer2 me-2"></i>Admin Dashboard</a></li>` : ''}
                        <li><a class="dropdown-item py-2" href="${safeBaseUrl}dashboard.html"><i class="bi bi-grid me-2"></i>My Dashboard</a></li>
                        <li><a class="dropdown-item py-2" href="${safeBaseUrl}checkout.html"><i class="bi bi-bag-check me-2"></i>My Purchases</a></li>
                        <li><a class="dropdown-item py-2" href="${safeBaseUrl}course/index.html"><i class="bi bi-mortarboard me-2"></i>My Learning</a></li>
                        <li><a class="dropdown-item py-2" href="${safeBaseUrl}ai/index.html"><i class="bi bi-cpu me-2"></i>AI Usage</a></li>
            ...
            container.innerHTML = `
                <div class="auth-buttons d-flex gap-2">
                    <a href="${safeBaseUrl}login.html" class="btn btn-outline-light btn-sm"><i class="bi bi-shield-lock me-1"></i>Login</a>
                    <a href="${safeBaseUrl}signup.html" class="btn btn-info btn-sm"><i class="bi bi-person-plus me-1"></i>Sign Up</a>
                </div>
            `;
        }
    }
